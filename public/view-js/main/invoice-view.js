...
populateSalesInvoiceList = function () {
  ...
  showLoader();
  $.post("/api/sales/getinvoices", PostObject, function (data, status) {
    if (data.length > 0) {
      for(x=0; x<data.length; x++){
        delete (data[x].CreatedBy.ProfilePicture);
      }
      $("#templatelist").show();
...
onmouseup = function (e) {
  if (typeof e.model !== "undefined") {
    ...
    //     ]
    //   : [];
    global_print_object.Customer = e.model.dataSource[e.index].Customer
      ? [
          {
            Name: e.model.dataSource[e.index].Customer.Name,
            Phone: e.model.dataSource[e.index].Customer.Phone,
            Address: e.model.dataSource[e.index].Customer.Address,
          },
        ]
      : [];
    ...
    if (e.model.dataSource[e.index].Customer != null) {
      $('#invoice-number').html(`Invoice no: ${e.model.dataSource[e.index].Name}`);
